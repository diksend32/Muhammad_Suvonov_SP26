...
SELECT  t.calendar_week_number, SUM(s.amount_sold) AS sales, 
		SUM(SUM(s.amount_sold)) OVER(ORDER BY t.calendar_week_number ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS for_rows,
		SUM(SUM(s.amount_sold)) OVER(ORDER BY t.calendar_week_number RANGE BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS for_range,
		SUM(SUM(s.amount_sold)) OVER(ORDER BY t.calendar_week_number GROUPS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS for_groups
FROM	sh.sales s
JOIN	sh.times t
		ON t.time_id = s.time_id
