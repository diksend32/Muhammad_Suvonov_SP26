...
-- Task 1
-- I used DENSE_RANK for not skipping the rows wiht the same amount_sold
WITH cte1 AS(
SELECT	ch.channel_desc, c.cust_first_name, c.cust_last_name, SUM(s.amount_sold) AS amount_sold,
        DENSE_RANK() OVER(PARTITION BY ch.channel_desc ORDER BY SUM(s.amount_sold) DESC) AS l
FROM	sh.customers c
JOIN    sh.sales s
			ON c.cust_id = s.cust_id
JOIN sh.channels ch
			ON ch.channel_id = s.channel_id
GROUP BY ch.channel_desc, c.cust_first_name, c.cust_last_name),
cte2 AS(
SELECT  ch.channel_desc, SUM(s.amount_sold) AS total
FROM	sh.channels ch
JOIN  sh.sales s
			ON ch.channel_id = s.channel_id
...
SELECT  c1.channel_desc, c1.cust_first_name, c1.cust_last_name, c1.amount_sold AS amount_sold,  
		ROUND((c1.amount_sold*100/c2.total), 4) || ''|| '%' AS sales_percentage
FROM cte1 c1
JOIN cte2 c2
		ON c1.channel_desc = c2.channel_desc
...
SELECT  ch.channel_desc, c.cust_id, c.cust_last_name, c.cust_first_name, t.calendar_year,
		DENSE_RANK() OVER(PARTITION BY ch.channel_desc, t.calendar_year ORDER BY SUM(s.amount_sold) DESC) AS ranking,
...
SELECT  c.channel_desc, c.cust_id, c.cust_last_name, c.cust_first_name, SUM(total) AS amount_sold
FROM	cte c
WHERE c.ranking <=300
GROUP BY c.channel_desc, c.cust_id, c.cust_last_name, c.cust_first_name
HAVING COUNT(DISTINCT c.calendar_year) = 3

-- Task 4
SELECT   TO_CHAR(t.time_id, 'YYYY-MM')  AS calendar_month_desc, p.prod_category, 
		SUM(s.amount_sold) FILTER (WHERE cr.country_region = 'Americas') OVER(PARTITION BY TO_CHAR(t.time_id, 'YYYY-MM'), p.prod_category) AS "Americas Sales",
		SUM(s.amount_sold) FILTER (WHERE cr.country_region = 'Europe') OVER(PARTITION BY TO_CHAR(t.time_id, 'YYYY-MM'), p.prod_category) AS "Europe Sales"
FROM	sh.products p
JOIN	sh.sales s
