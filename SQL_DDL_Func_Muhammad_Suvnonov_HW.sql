...
-- 1. I filtered this qaurter as    p.payment_date >= DATE_TRUNC('quarter', CURRENT_DATE) AND p.payment_date < DATE_TRUNC('quarter', CURRENT_DATE) + INTERVAL '3 months'. It enforce 
...
WHERE   p.payment_date >= DATE_TRUNC('quarter', CURRENT_DATE) AND p.payment_date < DATE_TRUNC('quarter', CURRENT_DATE) + INTERVAL '3 months'
GROUP BY c.name ;
...
CREATE OR REPLACE FUNCTION get_sales_revenue_by_category_qtr (IN year_ INT, quarter INT)
...
WHERE	EXTRACT(YEAR FROM p.payment_date) = year_ 
		AND  p.payment_date >= MAKE_DATE(year_,(quarter - 1) * 3 + 1, 1)
		AND  p.payment_date <  MAKE_DATE(year_, (quarter - 1) * 3 + 1, 1)+ INTERVAL '3 months' 
GROUP BY c.name 
$$
LANGUAGE sql;
...
RETURN QUERY
WITH cte1 AS(
SELECT 	cn.country, f.film_id f.title, f.rating, l.name, f.release_year, COUNT(r.rental_id) as total
FROM	public.film f
JOIN	public.inventory i
...
cte2 AS(
SELECT	*, RANK() OVER(PARTITION BY c1.country ORDER BY c1.total DESC, c1.film_id, c1.title) as r
FROM	cte1 c1)

SELECT c2.country, c2.title, c2.rating, c2.name as language, c2.release_year
FROM  cte2 c2
WHERE r = 1 AND  UPPER(c2.country) = ANY(UPPER(p_countries));
END;
$$;
