...
USING(
	customer_id IN (
			...
			WHERE  first_name = current_user -- I used just first name
			)
)
...
CREATE ROLE Muhammad LOGIN PASSWORD '123'

-- Giving the privileges is depersonalized
CREATE ROLE customer;  
GRANT SELECT ON rental, payment, customer TO customer;
GRANT customer TO Muhammad;

-- switching to Muhammad
SELECT * 
FROM   public.rental;
