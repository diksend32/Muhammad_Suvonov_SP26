...
SELECT	c.store_id, c.total_revunue,
		CASE
			WHEN NULLIF(a.address2, '') IS NOT NULL
...
SELECT	t.store_id, t.total_revunue,
		CASE
			WHEN NULLIF(a.address2, '') IS NOT NULL
			THEN a.address || ', ' || a.address2
			ELSE a.address
...
SELECT	 s.first_name, s.last_name,  a.address, s.store_id, MAX(p.payment_date) as last_one,  SUM(p.amount) as total_revenue
...
GROUP BY  s.first_name, s.last_name,    a.address, s.store_id
ORDER BY 	total_revenue desc
LIMIT 3;
-- Subquery

SELECT	ps.first_name, ps.last_name,  ps.address, ps.store_id, MAX(ps.payment_date) as last_one,  SUM(ps.amount) as total_revenue
FROM	
		(SELECT	 s.first_name, s.last_name,  a.address, s.store_id, p.payment_date, p.amount 
		 FROM	public.payment p
		 INNER JOIN	public.staff s
					ON p.staff_id = s.staff_id
		 INNER JOIN public.address a
...
GROUP BY  ps.first_name, ps.last_name,    ps.address, ps.store_id
ORDER BY 	total_revenue desc
LIMIT 3;
...
ORDER BY pause_year DESC;
 ...
 --V2  
-- JOIN SUBQUERY
SELECT  a.actor_id, a.first_name, a.last_name,	MAX(f2.release_year - f1.release_year) AS max_gap
FROM	public.film f1
INNER JOIN public.film_actor fa1
 		ON f1.film_id = fa1.film_id
INNER JOIN public.actor a
			ON a.actor_id = fa1.actor_id
INNER JOIN public.film_actor fa2
			ON fa2.actor_id = a.actor_id
INNER JOIN public.film f2
			ON f2.film_id = fa2.film_id
WHERE f2.release_year > f1.release_year
AND NOT EXISTS (
				SELECT 1
				FROM	public.film f3
				INNER JOIN public.film_actor fa3
						ON fa3.film_id = f3.film_id
				WHERE fa3.actor_id = a.actor_id
				AND f3.release_year > f1.release_year
				AND f2.release_year > f3.release_year
...
--CTE
WITH cte AS (
			SELECT  a.actor_id, a.first_name, a.last_name,	f2.release_year AS year2, f1.release_year AS year1
			FROM	public.film f1
			INNER JOIN public.film_actor fa1
			 		ON f1.film_id = fa1.film_id
			INNER JOIN public.actor a
...
SELECT 	 c.actor_id, c.first_name, c.last_name,	MAX(c.year2 - c.year1) AS max_gap
FROM	cte c
WHERE NOT EXISTS (
    SELECT 1
    FROM public.film_actor fa3
    INNER JOIN public.film f3
        ON f3.film_id = fa3.film_id
    WHERE fa3.actor_id = c.actor_id
      AND f3.release_year > c.year1
      AND f3.release_year < c.year2
)
